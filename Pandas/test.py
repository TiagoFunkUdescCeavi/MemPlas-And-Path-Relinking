...
best = pd.read_csv("../Logs/test_2.csv")
print( '>>>>>>>>> Best' )
print( best )

# Junto os dados com as melhores soluções
together = pd.merge(data, best, how ='inner', on ='key')
print( '>>>>>>>>> Together' )
print( together )

# Calculo uma nova coluna com o valor da diferença entre o valor e a melhor solução.
together['difference'] = together['value']-together['best'];
print( '>>>>>>>>> Together' )
print( together )
...
final = pd.merge(final, number_bests, how ='inner', on ='key')
# final.columns = ['key','mean','min']
print( '>>>>>>>>> Final' )
print( final )

# Junto com os valores.
final = pd.merge(best, final, how ='inner', on ='key')
print( '>>>>>>>>> Final' )
print( final )
